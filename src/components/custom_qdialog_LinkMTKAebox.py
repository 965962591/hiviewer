...
is_standalone = os.path.basename(sys.argv[0]).lower() == "custom_qdialog_LinkMTKAebox.py"
...
class MTK_Dialog(QDialog):
    ...
    def setShortcut(self):
        ...
        self.load_button.clicked.connect(self.load_mtk_path)         
    ...
    def init_ui(self):
        ...
        # 设置窗口标题，窗口大小，窗口图标
        self.setWindowTitle("MTK(DebugParser)工具解析图片接口")
        self.setFixedSize(1200, 300)
        self.setWindowIcon(QIcon(os.path.join(BasePath, "resource", "icons", "viewer_3.ico")))
        
        # 设置保存的json路径
        self.json_path = os.path.join(BasePath, "cache", "MTK_exif_settings.json")
        ...
        self.label1 = QLabel("MTK工具路径:", self)
        ...
        self.text_input1.setPlaceholderText(r"如:C:\D:\Tool_ml\DebugParserV4\DebugParserV4\DebugParser.exe")  # 设置提示文本
    ...
    def test_button1(self):
        ...
        if not os.path.exists(tool_path):
            ...
            self.status_button1.setText("❌")
            self.status_button1.setToolTip(f"❌当前路径不存在，请加载正确的锐IQT工具路径")
            return
    ...
    def get_url(self):
        """获取锐IQT工具路径和图片文件夹路径的编码url"""

        # 高通工具路径url
        mtk_path = self.text_input1.text()
        if mtk_path and os.path.exists(mtk_path):
            # url编码
            mtk_path_encoded = urlencode_folder_path(mtk_path)
            mtk_path_url = f"http://127.0.0.1:8000/set_c7_path/{mtk_path_encoded}"
        else:
            return []
    ...
    def load_mtk_path(self):
        """加载锐IQT工具路径"""
        try:
            options = QFileDialog.Options()
            file_path, _ = QFileDialog.getOpenFileName(self, "选择展锐IQT工具路径", "", "EXE Files (*.exe);;All Files (*)", options=options)
            if file_path:
                self.text_input1.setText(file_path)  # 显示选定的文件路径
        except Exception as e:
            print(f"选择锐IQT工具路径时发生错误: {e}")
    ...
    def save_settings(self):
        ...
        try:
            settings = {
                "展锐IQT工具路径": self.text_input1.text(),
            ...
            print("MTK_Dialog类_配置已保存")
        except Exception as e:
            print(f"MTK_Dialog类_保存配置失败: {e}")
    ...
    def load_settings(self):
        ...
        try:
            ...
            print("MTK_Dialog类_配置已成功读取")
        except Exception as e:
            print(f"MTK_Dialog类_读取配置失败: {e}")
